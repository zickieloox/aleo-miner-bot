...
console.log(argv.SERVER_IP, argv.SERVER_ID, argv.COMMAND_METHOD)
...
async function startMiner() {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            let ALEO_COMMAND = `./damominer --address ${address} --proxy asiahk.damominer.hk:9090`

            if (argv['g'] != undefined) {
                if (!Array.isArray(argv['g'])) {
                    argv['g'] = [argv['g']]
                }
                for (const g of argv['g']) {
                    ALEO_COMMAND += ' -g ' + g 
                }
            } else {
            ...
            // if (argv['proxy']) {
            //     ALEO_COMMAND = ALEO_COMMAND.split('[proxy]').join(address)
            // } else {
